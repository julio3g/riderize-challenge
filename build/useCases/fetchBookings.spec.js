...
var FetchBookingsUseCase = class {
  constructor(bookingsRepository2) {
...
// src/useCases/fetchBookings.spec.ts
...
describe("Fetch Bookings Use Case", () => {
  ...
  it("should be able to list rides", async () => {
    ...
    globalExpect(bookings).toHaveLength(2);
    globalExpect(bookings).toEqual([
      globalExpect.objectContaining({ rideId: "ride-id-1" }),
      globalExpect.objectContaining({ rideId: "ride-id-2" })
    ]);
  });
});
